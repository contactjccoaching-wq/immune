...
async function cmdRetrievalTest() {
  ...
  const tests = [
    ...
    { query: 'credentials secret management', domains: '["code"]', type: 'antibody',
      expect_contains: 'AB-CODE-022', desc: 'Should find hardcoded credentials pattern' },
    { query: 'protect sensitive service access', domains: '["code"]', type: 'strategy',
      expect_contains: 'CS-CODE-024', desc: 'Should find loopback binding strategy' },
    { query: 'monitoring health Docker services', domains: '["code"]', type: 'strategy',
      expect_contains: 'CS-CODE-025', desc: 'Should find HTTP healthcheck strategy' },
    { query: 'save money API costs', domains: '["code","strategy"]', type: 'strategy',
      expect_contains: 'CS-STRATEGY-001', desc: 'Should find claude-cli backend strategy' },
    { query: 'XSS injection template user data', domains: '["code"]', type: 'antibody',
      expect_contains: 'AB-CODE-023', desc: 'Should find server-side template XSS' },
    { query: 'authentication before expensive operations', domains: '["code"]', type: 'strategy',
      expect_contains: 'CS-CODE-016', desc: 'Should find auth-before-API-call strategy' },
  ];
